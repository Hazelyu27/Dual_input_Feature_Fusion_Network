...
# 没用kd
def train_temp(train_loader, network, criterion, optimizer):
    ...
    for (features, target) in train_loader:
        ...
        if isfirst == True:
            ...
            isfirst = False
            # reproject
            history_features = reproject(outputs)
        else:
            # 将history_features 和当前帧放到权重学习层里
            temp_weights = weight_learning_layer(history_features,features)
            outputs = outputs * temp_weights + history_features * (1- temp_weights)
