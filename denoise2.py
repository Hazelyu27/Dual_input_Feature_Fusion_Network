...
def denoise(add_bg = False,save_exr = True):
    ...
    model.eval()
    input1 = torch.ones(1,3,256,256).to(device)
    input2 = torch.ones(1,3,256,256).to(device)
    ...
    with (torch.no_grad()):
        for features,target in test_loader:
            ...
            alpha = target[:, 6:, :, :].numpy()
            background = target[:,3:6,:,:].numpy()
            alpha = np.concatenate((alpha, alpha, alpha), axis=1)
            output = output.cpu().numpy()
            # target = target.numpy()
            target = target[:,:3,:,:].numpy()
            for i in range(output.shape[0]):
                ...
                if add_bg == True:
                    ...
                    writeimage_with_bg2(output[i],background[i],alpha[i],outputfile,if_sharp=False)
                    writeimage_with_bg2(noisy_r[i],background[i],alpha[i],noisyfile)
                    writeimage_with_bg2(target[i],background[i],alpha[i],reffile)
                else:
...
# 用exr自带的背景，用于生成视频
# 主要改动是在datasets里加上了background的通道
